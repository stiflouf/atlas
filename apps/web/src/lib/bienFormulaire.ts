...
export function parseBienFormData(formData: FormData): NouveauBien {
  ...
  if (!Number.isFinite(surface) || surface <= 0) {
    throw new ErreurSaisie("Surface invalide : doit être strictement supérieure à 0.");
  }
  if (!Number.isFinite(pieces) || pieces <= 0) {
    throw new ErreurSaisie("Nombre de pièces invalide : doit être strictement supérieur à 0.");
  }
  if (!Number.isFinite(prix) || prix < 0) {
    throw new ErreurSaisie("Prix invalide : doit être positif ou nul.");
  }
  if (etage !== undefined && (!Number.isFinite(etage) || etage < 0)) {
    throw new ErreurSaisie("Étage invalide : doit être positif ou nul.");
  }
