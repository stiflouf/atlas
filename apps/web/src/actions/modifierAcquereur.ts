...
export async function modifierAcquereurAction(_etatPrecedent: EtatFormulaire, formData: FormData): Promise<EtatFormulaire> {
  ...
  return avecFeedbackFormulaire(async () => {
    ...
    const resultat = await getDb().transaction(async (tx) => {
      ...
      if (sourceCriteres.source === "projet") {
        ...
        if (!projet) throw new ErreurSaisie("Le Core refuse ces critères : budget minimum supérieur au maximum.");
