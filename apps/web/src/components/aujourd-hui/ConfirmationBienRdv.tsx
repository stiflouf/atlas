...
export default function ConfirmationBienRdv({
  ...
  if (etat === "confirme") {
    return (
      <div className="flex flex-col items-end gap-0.5">
        <p className="text-[11px] text-text-muted">Bien confirmé : {choisi.titre}</p>
        <Link
          href={`/visites/${rdvId}/preparer`}
          className="shrink-0 self-end min-h-[44px] flex items-center text-[13px] font-medium text-action-primary hover:text-action-primary-hover transition-colors"
        >
          Préparer&nbsp;→
        </Link>
      </div>
  ...
  if (etat === "choix") {
    return (
      <div className="flex flex-col items-end gap-1 text-[12px]">
        {candidats.map((c) => (
          <button key={c.bienId} onClick={() => valider(c, "corrige")} className="text-action-primary font-medium">
            {c.titre}
          </button>
        ))}
        <button onClick={ignorer} className="text-text-muted">
          Annuler
        </button>
      </div>
    );
  ...
  return (
    <div className="text-[12px] text-text-secondary max-w-[200px] text-right">
      <p className="mb-1.5 leading-snug">Ce rendez-vous concerne-t-il {candidats[0].titre} ?</p>
      <div className="flex flex-wrap justify-end gap-x-2 gap-y-1">
        <button onClick={() => valider(candidats[0], "confirme")} className="font-medium text-action-primary">
          Oui
        </button>
        {candidats.length > 1 && (
          <button onClick={() => setEtat("choix")} className="text-text-secondary">
