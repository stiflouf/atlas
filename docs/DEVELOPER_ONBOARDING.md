...
| **Écriture dans une table racine** | **ADR-054** — `workspace_id` est obligatoire et sans `DEFAULT` (migration `0033`). Le périmètre vient de `exigerWorkspaceCourant()` (requête authentifiée) ou de `resoudreWorkspaceExecutionMachine()` (cron/scan) — jamais d'un littéral dans un repository, règle verrouillée par `src/db/appartenanceWorkspace.structurel.test.ts` |
