...
| **Création d'une nouvelle table** | **ADR-054** — toute table nouvelle porte son appartenance dès sa création (racine avec `workspace_id`, ou feuille avec FK NOT NULL vers un parent possédé) |
| **Écriture dans une table racine** | **ADR-054** — `workspace_id` est obligatoire et sans `DEFAULT` (migration `0033`). Le périmètre vient de `exigerWorkspaceCourant()` (requête authentifiée) ou, pour un traitement machine, du workspace en cours de traitement : depuis WORKSPACE_SCOPING_V2B5, les scanners temporels parcourent `listerTousLesWorkspaceIds()` et exécutent chaque règle dans son workspace (`resoudreWorkspaceExecutionMachine()` ne sert plus qu'à `bootstrapperAppartenanceOwner` et ne fait plus partie du chemin des scanners) — jamais d'un littéral dans un repository, règle verrouillée par `src/db/appartenanceWorkspace.structurel.test.ts` |
| Contacts, personnes, projets vendeur/acquéreur, mandats, interactions | ADR-027, 055 |
| Connecteur, import, synchronisation, provenance d'une donnée externe | ADR-005, 006, 056 — et `docs/PLATFORM_BOUNDARIES.md` pour situer la couche |
